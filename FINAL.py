...
def generate_and_show_qrcode():
    ...
    # Gerar o QRCode com base no texto inserido
    box_size = 4
    imagem = qrcode.make(text, box_size=box_size)
...
imagem_fundo = choice(["fundosombra.jpg"])
imagem = Image.open(imagem_fundo)
imagem = imagem.resize((1000, 1000), Image.ANTIALIAS)
...
# Criar entrada de texto para o QRCode personalizado
text_entry_label = tk.Label(root, text="Texto para QRCode:")
text_entry_label.pack(pady=10)
text_entry = tk.Entry(root)
text_entry.pack()

# Criar botão para gerar o QRCode personalizado
generate_qrcode_button = tk.Button(
    root, text="Gerar QRCode", command=generate_and_show_qrcode)
generate_qrcode_button.pack(pady=10)
...
reset_button = tk.Button(root, text="Resetar", command=reset_password)
